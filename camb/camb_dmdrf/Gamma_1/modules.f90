    ...
    if (present(error)) then
        ...
        write(*,'("Gamma0(10^-7 Mpc^-2) = ",f7.2)') CP%Gamma0*1.e7              !drf
        if (CP%Num_Nu_Massive > 0) then
...
500     if (L< lmax_extrap_highl) &
    ...
    public thermo_dark, Thermo_OpacityToTime_dark, inithermo_dark, tight_tau_drf, tight_tau_idm
    contains 

    subroutine thermo_dark(tau, opacity, dopacity)
    ...
    function  Thermo_OpacityToTime_dark(opacity)
      real(dl), intent(in) :: opacity
      integer j
      real(dl) Thermo_OpacityToTime_dark

      !ZP: scat_drf ~ a**(2-Gpwr)  [~ a  for Gpwr=1]
      j = nthermo_dark
    ...
    tight_tau_drf= CP%tau0
    tight_tau_idm= CP%tau0
    ...
    do i=2,nthermo_dark
       ...
       ! For Gamma_t ~ T^\Gpwr with \Gpwr = 1, tau_drf is larger than tau at early time 
       ! tau_drf = 0.005 tau_Hub is the onset of tight coupling

       !Determine tight-coupling switch for drf
       if (tight_tau_drf==CP%tau0 .and. 1._dl/(tau*dotmu(i)) < 0.005) tight_tau_drf = tau
