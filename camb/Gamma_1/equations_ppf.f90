...
1   if (khi-klo.gt.1) then
    ...
    type EvolutionVars
        ...
        real(dl) TightSwitchOnTime_dark
    ...
    next_switch = min(tau_switch_ktau, tau_switch_nu_massless,EV%TightSwitchOnTime_dark,EV%TightSwitchoffTime, tau_switch_nu_massive, &
    ...
    if (next_switch < tauend) then
        ...
        !ZP idm-drf
        if (next_switch == EV%TightSwitchOnTime_dark) then 

            EVout%TightCoupling_dark=.true.
            EVout%TightSwitchOnTime_dark = noSwitch
    ...
    if (CP%WantScalars) then
        ...
        EV%TightCoupling_dark=.false.  !ZP idm-drf dark tightcoupling turns on at late time
    ...
    dgrho = dgrho + grhog_t*clxg+grhog_drf_t*clxg_drf+grhor_t*clxr !ZP drf contribution
    dgq   = dgq   + grhog_t*qg+grhog_drf_t*qg_drf+grhor_t*qr 
    ...
    !ZP idm-drf  scat_drf ~ a for Gpwr=1, tight coupling turns on at late time
    EV%TightSwitchOnTime_dark = max(tight_tau_drf, Thermo_OpacityToTime_dark(EV%k_buf/0.005))
    ...
    !ZP  adotoa = a*H, (adotoa)^2 = a^2 * H^2 = 8*pi*rho* a^2
    ...
    !ZP drf
    grho = grho_matter+grhor_t+grhog_t+grhog_drf_t+grhov_t
    ...
    dgrho=dgrho + grhog_t*clxg+grhog_drf_t*clxg_drf+grhor_t*clxr  !ZP drf

    !  8*pi*a*a*SUM[(rho_i+p_i)*v_i]
    dgq=dgq + grhog_t*qg+grhog_drf_t*qg_drf+grhor_t*qr      !ZP drf
    ...
    if (EV%TightCoupling_dark) then 
       ...
       Gamma_t = CP%Gamma0/a**CP%Gpwr                  !ZP idm-drf Gamma_t = Gamma0/a**Gpwr
